...
from swagger_server.models import commeent, movie, review, tvshow, title
...
def add_review(review: review) -> None:
  ...
  with connection.cursor() as cursor:
    sql = f"INSERT INTO review (title, content, rating, date) VALUES " \
          f"(%s, %s, %s, %s)"
    cursor.execute(sql, (review.title, review.content, review.rating, review.date))
    connection.commit()
